...
public class HostBlackListsValidator {
	...
	LinkedBlockingQueue<Integer> blackListOcurrences;
	private int cantSer;
     ...
     */
    public LinkedBlockingQueue<Integer> checkHost(String ipaddress, int cantidadThread) throws InterruptedException{
        skds = HostBlacklistsDataSourceFacade.getInstance();
        blackListOcurrences = new LinkedBlockingQueue<>();
        ...
        //Primero creamos todos los hilos y ejecutamos su m�todo run, de manera que corran en paralelo.
    	for (SearchThread h : hilos) {
    		h.start();
    	}
    	//A todos los hilos ejecutamos el m�todo join, para que el hilo principal no termine si alg�n hilo falta por terminar.
    	for (SearchThread h : hilos) {
    		h.join();
    	}
